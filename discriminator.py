...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        nn.Conv2d(2, 64, 6, stride=2, padding=2, bias=False), # in channel, out channel, filter kernel size
        nn.BatchNorm2d( 64 ),
        nn.LeakyReLU( 0.2 )
        )
        ...
        nn.Conv2d(256, 512, 4, stride=2, padding=1, bias=False), # in channel, out channel, filter kernel size
        nn.BatchNorm2d( 512 ),
        nn.LeakyReLU( 0.2 )
        )
...
if __name__ == "__main__":
    ...
    print(img_lab)
    discriminator(torch.tensor(  [np.transpose(img_lab[:, : ,[1,2]])]  ).float())

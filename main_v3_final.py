...
def optimizers(generator, discriminator, learningrate = 2e-4, amsgrad = False, b = 0.7, momentum = 0.9):
    ...
    Discriminator_optimizer = optim.Adam(
        generator.parameters(), lr = lr_disc, betas=(b, 0.999))
...
def GAN_training():
    ...
    generator = generator = VanillaGenerator()  # ngpu) #.to(device) add later to make meory efficient
    ...
    #torch.save(generator.state_dict(), path + "models/bajs.pt")
    #input("dsfdfg")
    reference_list = []
...
# image_norm, image_gray, image_gray_un_norm, image_rgb, img_name, img_name_gray
            ...
            if current_batch == 400:
                ...
                np.save(path +'G_losses_resumed_epoch30.npy', G_losses)
                np.save(path +'D_losses_resumed_epoch30.npy', D_losses)
                np.save(path +'G_losses_val_resumed_epoch30.npy', G_losses_val)
                np.save(path +'D_losses_val_resumed_epoch30.npy', D_losses_val)

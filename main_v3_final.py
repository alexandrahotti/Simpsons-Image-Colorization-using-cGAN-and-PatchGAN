...
def optimizers(generator, discriminator, learningrate = 1e-4, amsgrad = False, b = 0.7, momentum = 0.9):
    """ Returns an Adam optimizer for the discriminator and an Adam optimizer for the generator
        Using an intermediary value of 0.7 inbetween the default value of 0.9 and the value of 0.5
        from: https://arxiv.org/pdf/1803.05400.pdf, for the first parameter of betas
    """

    Discriminator_optimizer = optim.Adam( generator.parameters(), lr = learningrate, betas = (b, 0.999))
    Generator_optimizer = optim.Adam( generator.parameters(), lr = learningrate, betas = (b, 0.999))
...
def GAN_training():
    ...
    network_v = "network_v2_final"

    path = "/home/projektet/" + network_v +"/"

    # Create a genenerator and a discriminator
    generator = generator = VanillaGenerator().to(device)
    ...
    dataloader, batch_size, dataloader_validation = loadSimpsonDataset( workers, batch_size )
    ...
    #Discriminator_optimizer = torch.load("/home/projektet/network_v2_final/models/Discriminator_optimizer_400_29.pt")
    #Generator_optimizer = torch.load("/home/projektet/network_v2_final/models/Generator_optimizer_400_29.pt")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for current_batch,((image_norm, image_gray, image_gray_un_norm, image_rgb, img_name, img_name_gray), (image_norm_val, image_gray_val,image_gray_un_norm_val, image_rgb_val , img_name_val, img_name_gray_val)) in enumerate(zip(dataloader, dataloader_validation)):
            ...
            # Set the device depending on if it is cpu or cuda

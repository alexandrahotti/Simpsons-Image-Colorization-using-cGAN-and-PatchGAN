...
def compute_soft_min_val(patch_region_likelihoods):
    # Source: https://arxiv.org/pdf/1510.02364.pdf
    ...
    # input(log_likelihood.shape)
    # Calculating the weights

    exp_log_likelihood  = torch.exp(-alpha*log_likelihood)
    #print(exp_log_likelihood)
    # input(exp_log_likelihood.shape)

    exp_log_likelihood_sum = torch.sum( exp_log_likelihood, dim=3 )
    # print(torch.sum(exp_log_likelihood, dim=3).shape)
    #input(print(torch.sum(exp_log_likelihood, dim=3)))
    exp_log_likelihood_sum = torch.sum( exp_log_likelihood_sum, dim=2) # Summing all the weights for each image -> batch_size x 1

    #print(exp_log_likelihood_sum)
    # print(exp_log_likelihood_sum.unsqueeze(1).unsqueeze(1).shape)
    ...
    for epoch in range(0, epochs):
        ...
        #for current_batch,((image_norm, image_gray, image_rgb, img_name, img_name_gray), (image_norm_val, image_gray_val,image_rgb_val , img_name_val, img_name_gray_val)) in enumerate(zip(dataloader, dataloader_validation)):
            ...
            if min_patch_func:
                ...
                output = compute_soft_min_val(output)
            ...
            if min_patch_func:
                ...
                output_val = compute_soft_min_val(output_val)
            ...
            if min_patch_func:
                ...
                output1_val = compute_soft_min_val(output1_val)
            ...
            if min_patch_func:
                ...
                output2_val = compute_soft_min_val(output2_val)
